...
async def wiki_news_controller(request: WikiNewsRequest) -> WikiNewsResponse:
    ...
    headline, summary, content, imageUrl, isCompleted = await generate_wikinews_service(request.title, request.content, request.presignedUrl, request_id)

    # 표준 응답 스키마로 래핑하여 반환
    return WikiNewsResponse(
        message="뉴스 생성이 완료되었습니다.",
        data=WikiNewsData(headline=headline, summary=summary, content=content, imageUrl=imageUrl, isCompleted=isCompleted)
    )