...
# 앙상블 리트리버
def create_bm25_retriever(selected_docs):
    # 텍스트 기반 검색기
...
async def chat_service_stream(question: str, request_id: str, userId: int):
    ...
    if date_filter.get("mode") == "latest":
        selected_raw_docs = get_documents_by_qdrant(client, "ChoonChat")
        selected_docs = qdrant_records_to_documents(selected_raw_docs)
    else:
        selected_docs = header_splitted_docs

    retriever = create_ensemble_retriever(
    [create_bm25_retriever(selected_docs), create_qdrant_retriever(vectorstore)],
    weights=[0.3, 0.7]
    )
